...
class CreateSubjectUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        if (subjectId == -1L) return EntityWriteResult.DUPLICATE

        subjectRepository.replaceSubjectSemesters(subjectId, semesterIds)

        val defaultTypes = listOf(
            SubjectLessonType(id = 0, subjectId = subjectId, name = "Лекция", hours = null),
            SubjectLessonType(id = 0, subjectId = subjectId, name = "Практика", hours = null),
            SubjectLessonType(id = 0, subjectId = subjectId, name = "Лабораторная", hours = null)
        )
